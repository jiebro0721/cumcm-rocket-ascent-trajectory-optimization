...
图 \ref{fig:q3traj} 给出最优解对应的完整飞行曲线，图 \ref{fig:q3ctl} 给出俯仰角与节流比控制律。
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.9\textwidth]{fig_q3_controls}
\caption{问题三最优控制律}
\label{fig:q3ctl}
\end{figure}

由图 \ref{fig:q3traj} 可见：二子级点火点约在 2.5 min 处（一级燃尽后几乎不滑行），点火后高度由约 230 km 连续上升至 400 km，惯性速度由约 2600 m/s 增长至圆轨道速度 7672.6 m/s，飞行路径角在点火点附近约 21° 并逐渐减小至零，总质量由 80 t 匀速下降至 22.6 t（耗药 57.4 t）。俯仰角控制律（图 \ref{fig:q3ctl}a）呈"先降后升"的平滑曲线，由点火时 7.31° 下降至约 1.75° 后回升至 2.27°；节流比保持为满推力（图 \ref{fig:q3ctl}b）。

俯仰角由点火时约 $7.31^\circ$ 平滑下降，在约 $330$ s 处达到极小值 $1.75^\circ$，随后缓慢回升至关机点 $2.27^\circ$。这一“先降后升”的形态可由终端条件解释：推力方向在前期略高于水平，以同时爬升并补足切向速度；中期贴近水平以最小化径向速度残差；末端小幅上抬以将径向速度精确清零、同时补足微小的切向速度缺口。
...
最优控制策略可以完整表述为：\textbf{滑行时间 4.3764 s；二子级以额定推力点火，节流比在全程保持在 0.99998$\sim$1.00000 之间（图 \ref{fig:q4ctl}b 中 $\sigma$ 曲线紧贴上限 1.0），即节流能力在工程可忽略的意义下未被使用；俯仰角由点火时的 7.31° 平滑下降，约 330 s 处达到极小 1.75°，随后回升至关机点 2.27°}。与问题三的最优解相比，问题四仅把燃烧时间延长 0.0004 s、耗药增加 0.001 kg——节流自由度对最优轨迹的贡献趋近于零。
