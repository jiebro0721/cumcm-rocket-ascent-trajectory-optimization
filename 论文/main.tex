...
本文模型物理量纲一致、求解器与验证器统一（端到端复验残差小于 $10^{-6}$），各问结果可直接比较，可为多级运载火箭轨迹设计与推进剂优化提供参考。
...
表 \ref{tab:q3phi} 给出 20/40 网格最优解在燃烧段等距采样点处的俯仰角（由 41 个配点节点的分段线性插值重采样得到）。

\begin{table}[!htbp]
\caption{问题三最优俯仰角律（等距采样）}
\label{tab:q3phi}
\centering
\begin{tabular}{c|c|c|c|c|c|c|c|c}
\toprule[1.5pt]
$t-t_2$/s & 0 & 56.3 & 112.7 & 169.0 & 225.3 & 281.7 & 338.0 & 394.4 \\
\midrule[1pt]
$\phi$/° & 7.310 & 5.587 & 4.162 & 3.036 & 2.231 & 1.783 & 1.758 & 2.265 \\
\bottomrule[1.5pt]
\end{tabular}
\end{table}
...
节流后的动力方程及终端目标与问题三的区别仅在于：推力大小由 $\sigma(t)F_{\max2}$ 代替 $F_{\max2}$，质量流量相应缩放。求解流程与问题三完全一致，只是决策变量中增加 $N_b+1$ 个节流节点。
...
表 \ref{tab:q4sigma} 给出 20/40 网格最优解的节流比与俯仰角在燃烧段等距采样点处的数值（由 41 个配点节点的分段线性插值重采样得到）。

\begin{table}[!htbp]
\caption{问题四最优控制律（等距采样）}
\label{tab:q4sigma}
\centering
\begin{tabular}{c|c|c|c|c|c|c|c|c}
\toprule[1.5pt]
$t-t_2$/s & 0 & 56.3 & 112.7 & 169.0 & 225.3 & 281.7 & 338.0 & 394.4 \\
\midrule[1pt]
$\sigma$ & 0.999976 & 0.999999 & 1.000000 & 1.000000 & 1.000000 & 1.000000 & 1.000000 & 0.999989 \\
$\phi$/° & 7.310 & 5.587 & 4.162 & 3.036 & 2.231 & 1.781 & 1.752 & 2.265 \\
\bottomrule[1.5pt]
\end{tabular}
\end{table}

最优控制策略可以完整表述为：\textbf{滑行时间 4.3764 s；二子级以额定推力点火，节流比在全程保持在 0.99998$\sim$1.00000 之间（即节流能力在工程可忽略的意义下未被使用）；俯仰角由点火时的 7.31° 平滑下降，约 330 s 处达到极小 1.75°，随后回升至关机点 2.27°}。与问题三的最优解相比，问题四仅把燃烧时间延长 0.0004 s、耗药增加 0.001 kg——节流自由度对最优轨迹的贡献趋近于零。
