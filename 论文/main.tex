...
由图 \ref{fig:q3traj} 可见：二子级点火点约在 2.5 min 处（一级燃尽后几乎不滑行），点火后高度由约 230 km 连续上升至 400 km，惯性速度由约 2600 m/s 增长至圆轨道速度 7672.6 m/s，飞行路径角在点火点附近约 21° 并逐渐减小至零，总质量由 80 t 匀速下降至 22.6 t（耗药 57.4 t）。俯仰角控制律（图 \ref{fig:q3ctl}）呈"先降后升"的平滑曲线：由点火时 7.31° 平滑下降，在约 330 s 处达到极小值 1.75°，随后缓慢回升至关机点 2.27°。

这一"先降后升"的形态可由终端条件解释：推力方向在前期略高于水平，以同时爬升并补足切向速度；中期贴近水平以最小化径向速度残差；末端小幅上抬以将径向速度精确清零、同时补足微小的切向速度缺口。
...
\begin{itemize}
  ...
  \item \texttt{src/q2\_inscription.py}：问题二入轨条件三维打靶；
  \item \texttt{src/q3\_fuel\_opt.py}：问题三旧版单重打靶求解器，用于方法对比；
  \item \texttt{src/q4\_throttle\_opt.py}：问题四旧版单重打靶求解器，已修正节流容量约束；
  \item \texttt{src/q34\_direct\_collocation.py}：问题三、四 Hermite--Simpson 直接配点求解器；
  \item \texttt{tests/}：配点数学检查与模型约束回归测试；
  \item \texttt{results/}：全部数值结果（含配点网格收敛与独立重积分复验数据）。
\end{itemize}
全部程序均可通过 \texttt{python src/q34\_direct\_collocation.py --question 3 --n-coast 20 --n-burn 40}
等命令复现论文数值结果。
\end{appendices}
