...
\begin{abstract}
两级运载火箭的精准入轨与推进剂优化是航天工程的核心课题。本文针对赤道发射、目标为 400 km 近地圆轨道的两级液体运载火箭，建立了考虑地球自转、旋转大气阻力、变质量与级间质量跳变的统一多阶段动力学模型，并按照“给定控制传播—有限维参数反演—推力方向最优控制—方向与节流联合控制”的层级逐步开放决策自由度，使四问共享同一物理模型、终端流形与验证标准。

针对问题一，\textbf{建立了地心惯性系下的平面点质量动力学模型}，以赤道东射初速 $v_0=\omega_E R_E$ 刻画地球自转优势，以相对大气速度计算阻力，以真空比冲确定质量流量，并采用分段自适应数值积分附事件检测完成基准策略全程仿真。结果表明：二子级燃尽时高度 \textbf{433.53 km}、惯性速度 \textbf{8521.18 m/s}、飞行路径角 $-0.69^\circ$，对应近地点约 431 km、远地点约 4723 km 的椭圆轨道而非目标圆轨道。该结果直接说明基准策略燃料打过头，必须提前关机并优化控制律。

针对问题二，\textbf{将入轨条件写成半径、径向速度与顺行切向圆轨道速度三个等式，构建滑行时间、俯仰角角速率与二级燃烧时间组成的三维非线性打靶模型}，通过 196 组网格初值加 fsolve 求解。得到可行入轨解：滑行时间 \textbf{103.63 s}，角速率 \textbf{$-0.0474^\circ$/s}，剩余推进剂约 \textbf{4015 kg}，入轨残差达 $10^{-9}$ 量级。该问是可行解反演，无目标函数，属于参数搜索而非优化。

针对问题三，\textbf{将俯仰角由单一速率参数提升为可自由选取的时间函数，建立以推进剂消耗为目标、以状态轨迹与控制函数为决策变量的多阶段最优控制模型}，采用 Hermite--Simpson 直接配点将微分方程约束、阶段连接与终端流形同时转写为稀疏非线性规划，用 CasADi 自动微分与内点法求解。由于恒定满推力且比冲恒定，最省燃料等价于最短燃烧时间。20/40 区间网格求得滑行时间 \textbf{4.3767 s}、二级燃烧时间 \textbf{394.353 s}、推进剂消耗 \textbf{57446.93 kg}，与 10/20 网格仅差 0.012 kg；所得最优控制经独立高精度积分复验，终端高度误差小于 1 m。与问题二相比耗药减少约 538 kg，且最优滑行时间由 103.63 s 缩短至 4.38 s，说明旧的低维参数化限制了可行轨迹族，连续推力方向模型释放了新的控制自由度。

针对问题四，\textbf{在问题三基础上引入节流比 $\sigma(t)\in[0.6,1]$，并正确将推进剂容量约束改写为累计推力积分约束}，以同一配点转录求解。从满推力与 80\% 推力两类初值均收敛到同一近似满推力候选解（$\sigma$ 节点位于 0.99998$\sim$1.00000），滑行时间 4.3764 s，耗药 57446.93 kg，与问题三一致。本文结合固定比冲下的重力损失机理与文献结论指出，在无最大动压、过载、热流等路径约束的本题设定下节流不产生收益；进一步讨论了加入路径约束后节流可能出现的非平凡切换结构。鉴于问题非凸，第三、四问的结论均表述为经网格一致性与独立复验支持的局部最优候选解，不作全局最优性保证。
...
\end{equation}
并从网格多初值（196 组滑行时间、角速率与燃烧时间组合）出发作有界数值搜索。求得候选根后，用完整分段动力学重新传播并以米、米每秒量级的终端误差验收。求根器只是计算状态转移映射零点的工具，不构成该问的模型主体。
...
\subsection{连续模型及其结构}
令 $t_b=t_3-t_2$ 为二级燃烧时间。决策对象为滑行时间 $t_c$、燃烧时间 $t_b$、状态轨迹 $\bm x(t)$ 与俯仰角函数 $\phi(t)$。目标为最小化推进剂消耗；由于二级满推力且比冲恒定，目标等价于最小化燃烧时间（见下式）。约束分为三类：动力学方程、终端入轨条件与控制量边界。综上，问题三的标准模型为
\begin{equation}
\begin{aligned}
...
配点求解的完整流程如下：
\begin{enumerate}
  \item \textbf{建立决策变量}：滑行时间 $t_c$、燃烧时间 $t_b$、滑行段 $N_c+1$ 个状态节点、动力段 $N_b+1$ 个状态节点与 $N_b+1$ 个俯仰角节点；
  \item \textbf{施加约束}：一子级分离态固定为全部节点的初值；滑行段与动力段各自满足式 \eqref{eq:hs} 缺陷约束；动力段入口状态衔接滑行段出口；终端满足顺行圆轨道流形；
  \item \textbf{无量纲化与初始化}：位置、速度、质量分别以 $a$、$\sqrt{\mu/a}$、80000 kg 归一；以问题二的可行轨迹插值作为状态与控制初值；
  \item \textbf{求解}：先用粗网格 $(N_c,N_b)=(10,20)$ 求解，再将其解插值到细网格 $(20,40)$ 作为初值重解，并比较两网格的目标值差异；
  \item \textbf{独立复验}：将所得控制律（节点分段线性插值）交给与配点完全独立的 DOP853 积分器重新传播，检查终端高度、径向速度与切向速度误差。
\end{enumerate}

\subsection{结果与网格一致性}
表 \ref{tab:q3res} 给出两级网格结果。
...
\begin{enumerate}
  \item \textbf{初值一（满推力暖启动）}：直接取问题三的最优解，$\sigma_k\equiv1.0$，燃烧时间 $t_b=394.35$ s，作为第一个初值；
  \item \textbf{初值二（低节流独立启动）}：令 $\sigma_k\equiv0.8$，燃烧时间相应延长至约 $493$ s，状态轨迹以问题三最优解插值并人为适应更长燃烧段，作为第二个初值；
  \item \textbf{求解与网格加密}：两类初值分别在 $(10,20)$ 网格求解，再将各自解插值到 $(20,40)$ 网格重解，比较目标值；
  \item \textbf{独立复验}：将最优节流与俯仰角律（节点分段线性插值）交与独立的 DOP853 积分器重新传播，检查终端误差。
\end{enumerate}
两类初值在同一物理可行域内求解，避免依靠几十组盲目初值暴力搜索；若两类初值收敛到不同解，则可判断存在多个局部最优并考察其目标值差异。

\subsection{结果与分析}
表 \ref{tab:q4res} 给出两级网格结果。
...
在恒定节流这一控制族内，耗药随节流比单调增加：节流 0.9、0.8、0.7、0.6 分别使耗药增加 47.1、196.2、532.3、1279.4 kg。该定量结果直接量化了"降低推力延长燃烧时间、增加重力损失"的机理，与 §6 的理想速度增量分析（两级具备约 3134 m/s 速度余量）相互印证。需要说明的是，0.9$\sim$0.6 各档的最优滑行时间均收敛到 0 s 附近，说明低节流下"尽可能早点火、以长燃烧补足速度"成为局部最优形态；尽管如此，其耗药仍高于满推力解，进一步确认节流不带来收益。
...
\begin{appendices}
\section{支撑材料文件列表}
本论文的全部建模、求解与验证程序代码随支撑材料一并提交，文件列表如下：
\begin{itemize}
  \item \texttt{src/common.py}：物理常数、火箭参数、动力学方程、分段积分器与入轨残差；
  \item \texttt{src/q1\_baseline.py}：问题一基准控制策略全程仿真；
  \item \texttt{src/q2\_inscription.py}：问题二入轨条件有界三维打靶；
  \item \texttt{src/q34\_direct\_collocation.py}：问题三、四 Hermite--Simpson 直接配点求解器；
  \item \texttt{src/run\_all.py}：一键复现脚本（依次执行 Q1--Q4 并生成论文图表）；
  \item \texttt{legacy/shooting\_baseline/}：旧版单重打靶求解器与结果，用于方法对比；
  \item \texttt{cross\_validation/}：独立实现（RK4 + scipy 配点）的交叉验证代码、结果与说明（三档网格收敛、多初值审计、节流灵敏度、损失预算）；
